...
const Fetchcalendarmeetingswebpart = ({
  context,
}: {
  ...
  const getUserMeetings = async (): Promise<void> => {
    try {
      setloading(true);
      // Convert selected date to UTC for API request
      ...
      //AJ CODE SHOWING TIME IN UTC FORMAT
      // setloading(true);
      ...
      try {
        ...
        client
          ...
          .header("Prefer", "outlook.timezone=\"Asia/Karachi\"") // Add timezone header for Pakistan
          .get((err: any, res: any) => {
            if (err) {
            setloading(false);
          ...
          meetings.map((meeting: any, index: number) => (
            ...
            >
              <div className={styles.time}>
                {" "}
                {
                  new Date(meeting.start.dateTime)
                    .toLocaleTimeString("en-US", {
                      hour: "numeric",
                      minute: "2-digit",
                      timeZone: "Asia/Karachi" // Specify Pakistan timezone
                    })
                    .split(" ")[0]
                }
                <br />
                {
                  new Date(meeting.start.dateTime)
                    .toLocaleTimeString("en-US", { hour12: true,timeZone: "Asia/Karachi" })
                    .split(" ")[1]
                }
              </div>
...
export default Fetchcalendarmeetingswebpart;
...
//               <p>Start: {new Date(meeting.start.dateTime).toLocaleString()}</p>
//               <p>End: {new Date(meeting.end.dateTime).toLocaleString()}</p>
//               <p>Location: {meeting.location.displayName || 'Online'}</p>
//               <p>
//                 Attendees:{' '}
